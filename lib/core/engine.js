...
const Promise = require('bluebird'),
  ...
  startVideo = require('../support/video/startVideo'),
  stopVideo = require('../support/video/stopVideo'),
...
class Engine {
  ...
  run(url, scriptsByCategory, asyncScriptsByCategory) {
    ...
    if (get(options, 'experimental.video')) {
      options.preScript.push(startVideo);
      options.postScript.unshift(stopVideo);
    }
    ...
    function runIteration(index) {
      ...
      return runner.start()
        .tap(() => Promise.map(options.preScript, (preScript) => preScript.run(
          {
            url,
            options,
            log,
        ...
        .tap((results) => Promise.map(options.postScript, (postScript) => postScript.run(
          {
            url,
            options,
            log,
